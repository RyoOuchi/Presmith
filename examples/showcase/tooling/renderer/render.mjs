...
try {
  ...
  catch (error) { throw new Error(`Chromium cannot launch. Run presmith setup in this project. On Linux, install the Playwright system libraries. ${error.message}`); }
  if (input.action === 'probe') {
    result.success = true; result.runtime = {node:process.version, chromium:browser.version()};
    try { await import('./pptx.mjs'); result.capabilities={pptx_export:true}; }
    catch { result.capabilities={pptx_export:false}; }
  } else {
    let pptx;
    if (input.action === 'pptx') {
      try { pptx=await import('./pptx.mjs'); }
      catch (error) { throw new Error(`PPTX renderer unavailable. Run presmith setup --upgrade-renderer in this deck. ${error.message}`); }
    }
    ...
    for (const slide of selected) {
      ...
      if (pptx) {
        const scene=await pptx.captureSlide(page,slide);
        pptxScenes.push(scene);
        for (const warning of scene.warnings) result.findings.push({severity:'warning',slide_id:slide.id,source:slide.source,measurements:{},...warning});
      }
    ...
    if (!hasErrors) {
      if (pptx) {
        result.artifacts.push(await pptx.writePptx(pptxScenes,m,path.join(input.out,'deck.pptx')));
        result.findings.push({severity:'warning',rule_id:'pptx.portability',slide_id:null,element_id:null,source:'deck.json',
          message:'PPTX uses editable text line segments and basic shapes. Fonts and text metrics may differ in PowerPoint or Google Slides. Interactive content is static; PPTX edits do not sync to HTML. Speaker notes are included.',measurements:{}});
      }
      if (input.action === 'pdf') {
